...
for i=1:size(scs_m.objs)
      if typeof(scs_m.objs(i))=="Block" then
            if scs_m.objs(i).gui=="SUPER_f" then
                 mputl(["cJSON_AddItemToObject(json,"""+scs_m.objs(i).model.rpar.props.title+""",array=cJSON_CreateArray());"],fd);
                 for k=1:size(scs_m.objs(i).model.rpar.objs)
                      if typeof(scs_m.objs(i).model.rpar.objs(k))=="Block" then
                          if scs_m.objs(i).model.rpar.objs(k).gui~="IN_f" then
                              if scs_m.objs(i).model.rpar.objs(k).gui~="OUT_f" then
                                  if scs_m.objs(i).model.rpar.objs(k).gui~="SPLIT_f" then
                                     m=m+1;
                                  end
                              end
                          end
                 ...
                 mputl(["cJSON_AddItemToArray(array,obj=cJSON_CreateObject());"], fd);
                 for kf=n:m
                     mputl(["total_s=total_s+total_"+funs(kf)+"_"+string(kf)+";"], fd);
                 end
                 mputl([
                            "cJSON_AddNumberToObject(obj,""TOTAL_SUPERBLOCK:"",total_s);"], fd);
                 for kf=n:m
                     mputl([
                            "cJSON_AddNumberToObject(obj,"""+funs(kf)+"_"+string(kf)+":"",total_"+funs(kf)+"_"+string(kf)+");"], fd);
                 end
             elseif scs_m.objs(i).gui~="SUPER_f" then
                 if scs_m.objs(i).gui~="SPLIT_f" then
                    m=m+1;
                    n=m+1;
                    mputl(["cJSON_AddNumberToObject(json,"""+funs(m)+"_"+string(m)+":"",total_"+funs(m)+"_"+string(m)+");"], fd);
                 end
             end
       end
end
